                    ...
                    <div className="flex flex-col gap-22 align-middle max-h-full text-18 pt-26 overflow-auto hide-scrollbar w-full">
                        {
                            Object.entries(currentRoute).map(([key, value]) => {
                                ...
                                return (<div key={key} className="mx-auto">
                                    {value.Icon}
                                </div>)
                            })
                        }
                            ...
                            return (
                                <div key={key}>
                                    <div className='text-neutral-200 font-semibold flex align-middle gap-10 capitalize'>
                                        <div className='text-[1.5rem] mt-2'>
                                            {value.Icon}
                                        </div>
                                        {key}
                                    </div>
                                            ...
                                            value.links.map((item, index) => {
                                                return (
                                                    <Link to={item.link} key={index}>
                                                        <div key={index} className={`ml-6 capitalize hover:text-white hover:border-l-2 hover:border-l-blue transition-all text-14 cursor-pointer py-6 px-5 border-l  ${pathname === item.link ? 'border-l-2 border-l-blue text-blue font-semibold' : 'text-neutral-300 border-l-neutral-500 font-medium'}`}>{item.name}</div>
                                                    </Link>
                                                )
                                            })
                                        }
